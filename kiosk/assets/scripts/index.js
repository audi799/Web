...
$pay.querySelector(':scope > .button-container > .button.confirm').addEventListener('click', () => {
    if ($pay.querySelector(':scope > .method-container > .label > .radio[value="card"]').checked === true) {
        showDialog('현재 일시적으로 카드 결제가 불가능합니다. 카운터에 문의해 주세요.');
        return;
    }
    const date = new Date();
    ...
    imp.request_pay({
        ...
        if (resp.success === true) {
            ...
            // location.reload(); // 새로고침. 기존 장바구니부터 다 날라감.
        } else {
            ...
            $list.querySelectorAll(':scope > .item').forEach($item => {
                $item.addEventListener('click', () => {
                    ...
                    const calculateSum = () => {
                        /* 1번째 방법
                        let sum = 0;
                        for (const carObject of Object.values(cartMap)) {
                            sum += carObject['price'] * carObject['quantity'];
                        }
                        ...
                        $list.insertAdjacentHTML('beforeend', `
                                                                        <li class="item" data-name="${name}">
                                                                            ...
                                                                            <label class="quantity-wrapper">
                                                                                ...
                                                                                <input readonly class="quantity" type="text">
